...
with open("spot.csv","w",encoding="utf-8",newline="") as spot: # 開啟 CSV 檔案進行寫入
    ...
    for item1 in list1:
        ...
        image_urls = item1["filelist"].split(".jpg")  # 分割成多個部分
        image_urls = [url + ".jpg" for url in image_urls if url]  # 透過List Comprehension 列表生成式，加回 .jpg(使用列表生成式比傳統迴圈更簡潔、更具可讀性) "if url"過濾掉空字串或 None，只處理非空項目，因為 split() 可能會生成空字串。
        # 如果有多個 URL，取出第一個 URL
